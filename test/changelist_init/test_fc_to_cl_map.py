...
from test.changelist_init.conftest import get_sample_fc_path, create_fc
...
def test_offer_fc_into_cl_dict_scenario_0():
    assert not offer_fc_to_cl_dict({}, create_fc(get_sample_fc_path(0)))


def test_offer_fc_into_cl_dict_scenario_0_empty_fc_raises_exit():
    try:
        assert not offer_fc_to_cl_dict({}, FileChange())
        raised_exit = False
    except SystemExit:
        raised_exit = True
    assert raised_exit
